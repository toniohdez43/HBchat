...
public class OneMessage: NSObject {
    ...
    public class func sendMessage(message: String,  to receiver: String, completionHandler completion:OneChatMessageCompletionHandler) {
        ...
        /*if((img) != nil)
        {
            ...
            var imgStr:String! = data.base64EncodedStringWithOptions([])
            var imgAttachment:DDXMLElement! = DDXMLElement.elementWithName("attachment") as! DDXMLElement
            imgAttachment.setStringValue(imgStr)
            completeMessage.addChild(imgAttachment)
